...
public class DbMapperImpl<T> implements JdbcMapper<T> {
    ...
    public T findByLogin(String login, Class<T> clazz) {
        EntityClassMetaData<T> classMetaData = new ObjectDataExtractor<T>(clazz);
        EntitySQLMetaData constructor = new SQLContsructor(classMetaData);
        List<Field> fields = classMetaData.getAllFields();
        Object obj = null;
        try (PreparedStatement pst = connection.prepareStatement(constructor.getSelectByLoginSql())) {
    ...
    public List<T> findAll(Class<T> clazz) {
        ...
        Object obj = null;
        try (PreparedStatement pst = connection.prepareStatement(constructor.getSelectAllSql())) {
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    obj = classMetaData.getConstructor().newInstance();
                    for (Field field : fields) {
