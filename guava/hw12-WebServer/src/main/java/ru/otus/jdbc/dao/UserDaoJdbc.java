...
public class UserDaoJdbc implements UserDao {
    ...
    public Optional<User> findByLogin(String login) {
        try {
            return dbExecutor.executeSelect(getConnection(), "select id, name from user where login  = ?",
                    login, rs -> {
                        try {
                            if (rs.next()) {
                                return new User(rs.getInt("id"), rs.getString("name"), rs.getInt("age"), rs.getString("login"), rs.getString("pass"));
                            }
                        } catch (SQLException e) {
                            logger.error(e.getMessage(), e);
                        }
